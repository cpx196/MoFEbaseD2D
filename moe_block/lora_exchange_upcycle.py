    ...
    def experts_cosine_similarity(
        ...
        for i in range(self.num):
            for j in range(self.num):
                ...
                parts = []
                # combine linear1
                W1_comb = W1_base + torch.bmm(lr1_A[i].unsqueeze(0), lr1_B[j].unsqueeze(0)).squeeze(0)
                # combine linear2
                W2_comb = W2_base + torch.bmm(lr2_A[i].unsqueeze(0), lr2_B[j].unsqueeze(0)).squeeze(0)


                pre_linear1_w = W1_base + W1_comb
                pre_linear1_b = (b1_base + self.lr1_bias[i].to(device)) if b1_base is not None else None
                pre_linear2_w = W2_base + W2_comb
    ...
    def forward(self, x):
        ...
        for i in range(self.num):
            for j in range(self.num):
                ...
                # b1_base = self.base_expert.linear1.bias.to(device) if self.base_expert.linear1.bias is not None else None
                W1_comb = W1_base + torch.bmm(lr1_A[i].unsqueeze(0), lr1_B[j].unsqueeze(0)).squeeze(0)
                b1_comb = (b1_base + self.lr1_bias[expert_idx].to(device)) if b1_base is not None else None
                ...
                # b2_base = self.base_expert.linear2.bias.to(device) if self.base_expert.linear2.bias is not None else None
                W2_comb = W2_base + torch.bmm(lr2_A[i].unsqueeze(0), lr2_B[j].unsqueeze(0)).squeeze(0)
                b2_comb = (b2_base + self.lr2_bias[expert_idx].to(device)) if b2_base is not None else None
