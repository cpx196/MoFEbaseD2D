...
class Nystrom_MoE(nn.Module):
    ...
    def init_experts_from_checkpoint(self, state_dict, prefix: str):
        ...
        #     self.midmat_1[i].data.copy_(self.base_expert.linear1.weight.data[:self.rank, :self.rank])
        #     self.midmat_2[i].data.copy_(self.base_expert.linear2.weight.data[:self.rank, :self.rank])
        ...
        #     self.lr1_A[i].data.copy_(self.base_expert.linear1.weight.data[self.rank:, :self.rank])
        #     self.lr1_B[i].data.copy_(self.base_expert.linear1.weight.data[:self.rank, self.rank:])
        #     self.lr2_A[i].data.copy_(self.base_expert.linear2.weight.data[self.rank, :self.rank])
        #     self.lr2_B[i].data.copy_(self.base_expert.linear2.weight.data[:self.rank, self.rank:])
    ...
    def forward(self, x):
        ...
        for i in range(self.num_expert):
            ...
            b1_pre = b1_base + b1_comb[expert_idx] if b1_base is not None else b1_comb[expert_idx]
            t = F.linear(selected_inputs, W1_pre, b1_pre)
            t = self.activations[expert_idx](t)

            W2_pre = W2_base + W2_comb[expert_idx]
            b2_pre = b2_base + b2_comb[expert_idx] if b2_base is not None else b2_comb[expert_idx]
            out_i = F.linear(t, W2_pre, b2_pre)         # [n_i, c]
