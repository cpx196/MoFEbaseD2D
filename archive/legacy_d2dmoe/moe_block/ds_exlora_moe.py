...
class DS_EX_MoE(nn.Module):
        ...
        if self.share:
            for i in range(self.num):
                for j in range(self.num):
                    ...
                    delta_B1 = torch.bmm(lr1_A[i].unsqueeze(0), lr1_B[j].unsqueeze(0)).squeeze(0)  # [dim_feedforward, hidden_dim]
                    ...
                    delta_B2 = torch.bmm(lr2_A[i].unsqueeze(0), lr2_B[j].unsqueeze(0)).squeeze(0)  # [hidden_dim, dim_feedforward]
            ...
            for i in range(self.num):
                ...
                # combine linear1
                # delta_A1 = torch.bmm(lr1_A[i].unsqueeze(0), midmat_1[expert_idx].unsqueeze(0)).squeeze(0)  # [dim_feedforward, low_rank]
                delta_B1 = torch.bmm(lr1_A[i].unsqueeze(0), lr1_B[i].unsqueeze(0)).squeeze(0)  # [dim_feedforward, hidden_dim]
                W1_comb = W1_base + delta_B1
                # combine linear2
                # delta_A2 = torch.bmm(lr2_A[i].unsqueeze(0), midmat_2[expert_idx].unsqueeze(0)).squeeze(0)  # [hidden_dim, low_rank]
                delta_B2 = torch.bmm(lr2_A[i].unsqueeze(0), lr2_B[i].unsqueeze(0)).squeeze(0)  # [hidden_dim, dim_feedforward]
                W2_comb = W2_base + delta_B2
    ...
    def forward(self, x):
        ...
        if self.ec:
            ...
            delta_B1 = torch.bmm(final_lr1_A, final_lr1_B)
            # delta_A2 = torch.bmm(final_lr2_A, midmat_2)
            delta_B2 = torch.bmm(final_lr2_A, final_lr2_B)
