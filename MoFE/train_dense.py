...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--scheduler", choices=("cosine", "constant"), required=True)
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Restore model and optimizer state, then start the requested new schedule.",
    )
...
def main() -> None:
    ...
    if args.restart_scheduler_on_resume and not args.resume_from_checkpoint:
        raise ValueError("scheduler restart requires a resume checkpoint")
    ...
    accelerator.print(
        "training_config "
        f"model=dense num_processes={accelerator.num_processes} "
    ...
    if accelerator.is_main_process:
        ...
        (output_dir / "parameter_count.json").write_text(
            json.dumps(
                {
        ...
        for batch in dataloader:
            ...
            if global_step % args.logging_steps == 0 or global_step == 1:
                ...
                accelerator.print(
                    ...
                    f"peak_memory_gib={peak_memory / 2**30:.2f}",
                    flush=True,
                )
                if accelerator.is_main_process:
            ...
            if global_step % args.save_steps == 0:
                save_training_state(
                    accelerator, tokenizer, output_dir, global_step, args
                )
            if (
    ...
    if accelerator.is_main_process:
        ...
        (final_dir / "metadata.json").write_text(
            json.dumps(
                {"global_step": global_step, "training_args": vars(args)}, indent=2
            )
            + "\n"
        )
    accelerator.wait_for_everyone()
